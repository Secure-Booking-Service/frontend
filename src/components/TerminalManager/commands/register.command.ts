...
export const registerCommand: ICommand = {
  ...
  callback: async (manager, ...args) => {
    ...
    try {
      ...
      const attestationOptions: any = await store.dispatch("startAttestation", { username: email, token })
      ...
      await store.dispatch("finishAttestation", { username: email, token, attestationResponse });

      // 4. Successfully registered a new user!
      return manager.writeLine("Registration was successfull! You are now logged in as " + email);
